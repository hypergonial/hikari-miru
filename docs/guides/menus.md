...
=== "Gateway"
    ...
    class MainScreen(menu.Screen[miru.GW]):
        ...
        async def moderation(
            self, button: menu.ScreenButton[miru.GW], ctx: miru.ViewContext[miru.GW]
        ) -> None:
        ...
        async def fun(
            self, button: menu.ScreenButton, ctx: miru.ViewContext[miru.GW]
        ) -> None:
    ...
    class LoggingScreen(menu.Screen[miru.GW]):
        def __init__(self, menu: menu.Menu[miru.GW]) -> None:
            super().__init__(menu)
...
=== "REST"
    ...
    class MainScreen(menu.Screen[miru.REST]):
        # This method must be overridden in your screen classes
        # This is where you would fetch data from
        #  a database, etc. to display on your screen
        async def build_content(self) -> menu.ScreenContent:
        ...
        @menu.button(label="Moderation")
        async def moderation(
            self, button: menu.ScreenButton[miru.REST], ctx: miru.ViewContext[miru.REST]
        ) -> None:
            # Add a new screen to the menu stack,
            # the message is updated automatically
            await self.menu.push(ModerationScreen(self.menu))
        ...
        @menu.button(label="Logging")
        async def fun(
            self, button: menu.ScreenButton, ctx: miru.ViewContext[miru.REST]
        ) -> None:
            await self.menu.push(LoggingScreen(self.menu))
    ...
    class LoggingScreen(menu.Screen[miru.REST]):
        def __init__(self, menu: menu.Menu[miru.REST]) -> None:
            super().__init__(menu)
            # Your screens can store state in the class instance
            # But keep in mind that the instance will be
            # destroyed once the screen is popped off the stack
