...
- Added note about ``miru.load`` and ``miru.unload`` being removed in 2.2.0 to deprecation warning.
...
- Fixed ``Context.edit_response()`` typehints.
- Fixed navigator attachments being kept between page changes.

Version 2.0.1
=============

- Unblocked ``View.wait_for_input()`` when ``View.stop()`` is called.
